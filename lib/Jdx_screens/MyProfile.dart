...
class _DrawerScreenState extends State<DrawerScreen> {
  ...
  deleteAccount() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${ApiPath.baseUrl}authentication/delete_user'));
    request.fields.addAll({'user_id': userid.toString()});
    print("this is userId=========>${request.fields}");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: SingleChildScrollView(
          child: getprofile == null  || isLoading
              ? const Center(
                          ...
                          builder: (context){
                                                  ...
                                                  ? ClipRRect(
                                                      borderRadius:
                                                          BorderRadius.circular(
                                                              100),
                                                      child: Image.network(
                                                          "${getprofile!.data![0].userImage}"))
                                                  : Image.asset(
                                                      'assets/ProfileAssets/sampleprofile.png'),
                                            )
                                          : Container(
                                              width: 100,
                                              height: 100,
                            ...
                            InkWell(
                              ...
                              child: Container(
                                ...
                                child: Image.asset(
                                  'assets/ProfileAssets/editIcon.png',
                                  color: primaryColor,
                                  scale: 1.3,
                                ),
                              ),
